...
// Global DMA queue (initialized in commu.c via dma_manager_setup)
extern dma_queue_t *g_dma_queue;

// Global DMA queue for fetch worker thread (separate HW thread = separate DMA engine)
extern dma_queue_t *g_dma_queue_fetch;
...
static void transfer_permuted_weight_chunk_fp16(__fp16 *vtcm_dst, const __fp16 *src, int n_cols, int k) {
  ...
  // DMA queue-based transfer (matching llama.cpp hex-dma pattern)
  {
  ...
  if (!use_pipeline) {
    ...
    for (size_t mr = 0; mr < m; mr += m_chunk_n_rows) {
      ...
      // issue async DDR data transfer for the first weight chunk
      {
        ...
        dma_queue_push_ddr_to_vtcm_1d(g_dma_queue, dma_make_ptr(buf_curr, permuted_weight), first_weight_chunk_size);
      }
      ...
      for (size_t nc = 0; nc < n; nc += n_chunk_n_cols) {
        ...
        // int64_t wei_t0 = HAP_perf_get_qtimer_count();
        {
          dma_queue_pop(g_dma_queue);  // wait until current weight chunk become ready
          ...
          if (nc_next < n) {
            ...
            dma_queue_push_ddr_to_vtcm_1d(g_dma_queue, dma_make_ptr(buf_next, next_weight_chunk), next_weight_chunk_size);
          }
    ...
    for (size_t mr = 0; mr < m; mr += m_chunk_n_rows) {
      ...
      const size_t n_cols_A0 = smin(n - 0 * n_chunk_n_cols, n_chunk_n_cols);
      {
        ...
        dma_queue_push_ddr_to_vtcm_1d(g_dma_queue, dma_make_ptr(vtcm_qweight, qweight_chunk_A0), chunk_size_A0);
      }
      ...
      // prologue: B0, A1, C0, B1
      {
        ...
        if (1 < n_chunk_cnt) {
          ...
          dma_queue_push_ddr_to_vtcm_1d(g_dma_queue, dma_make_ptr(vtcm_qweight, qweight_chunk_A1), chunk_size_A1);
        }
      ...
      for (int i = 0; i < n_chunk_cnt; ++i) {
        ...
        if (i + 2 < n_chunk_cnt) {
          ...
          dma_queue_push_ddr_to_vtcm_1d(g_dma_queue, dma_make_ptr(vtcm_qweight, qweight_chunk_p2), chunk_size_p2);
        }
...
// Synchronously load 2D data from DDR to VTCM via DMA queue (single 2D transfer)
static void dma_load_2d_sync(uint8_t *dst, const uint8_t *src, size_t dst_stride, size_t src_stride, size_t height,
                             size_t width) {
  // NOTE: This runs on the fetch worker thread which has its own DMA engine.
  // Use g_dma_queue_fetch for the separate HW thread.
  dma_queue_push_ddr_to_vtcm_2d(g_dma_queue_fetch, dma_make_ptr(dst, src), width, height, src_stride, dst_stride);
      ...
      for (size_t kk = 0; kk < k; kk += K_BLOCK_SIZE) {
        ...
        // fetch activation block into VTCN
        {
          ...
          dma_queue_push_ddr_to_vtcm_2d(g_dma_queue,
                                        ...
                                        k_blk_sz * sizeof(float),   // width
                                        m_blk_sz,                   // height
                                        k * sizeof(float),          // src_stride
                                        k_blk_sz * sizeof(float));  // dst_stride
          dma_queue_pop(g_dma_queue);
        }
