...
// Global DMA queues (queue-based, matching llama.cpp hex-dma pattern)
...
AEEResult htp_ops_close(remote_handle64 handle) {
  ...
  // Destroy DMA queues
  if (g_dma_queue) {
    dma_queue_flush(g_dma_queue);
    dma_queue_delete(g_dma_queue);
    g_dma_queue = NULL;
  }
  if (g_dma_queue_fetch) {
    dma_queue_flush(g_dma_queue_fetch);
    dma_queue_delete(g_dma_queue_fetch);
    g_dma_queue_fetch = NULL;
  }
...
AEEResult htp_ops_init_backend(remote_handle64 handle) {
  ...
  // Create DMA queues (queue-based, matching llama.cpp hex-dma pattern)
  g_dma_queue       = dma_queue_create(64);
  g_dma_queue_fetch = dma_queue_create(64);
  if (!g_dma_queue || !g_dma_queue_fetch) {
    FARF(ALWAYS, "dma_queue_create failed");
